...
updateAt :: Int -> a -> [a] -> [a]
splitAt' :: Int -> [a] -> [[a]]
splitAt' i xs = let (xs',xs'') = splitAt i xs in [xs', tail xs'']
updateAt i x xs = let listOfLists = splitAt' i xs in listOfLists !! 0 ++ [x] ++ listOfLists !! 1
...
isPalindrome str 
            | length str == 0 = True
            | length str == 1 = True
            | length str == 2 = str !! 0 == str !! 1
            | otherwise = if (head str) == (last str) then isPalindrome (init (tail str)) else False
...
safeIndex xs i = if i < 0 then Nothing else if i > (length xs - 1) then Nothing else Just (xs !! i)
