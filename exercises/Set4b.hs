...
maxHelper candidate current = if candidate > current then candidate else current
...
slHelper val (curr,l) = (curr + val,l+1)
...
concatHelper :: [a] -> [a] -> [a]
concatHelper xs acc = foldr (\x a -> x : a) acc xs
...
largestHelper candidate [] = [candidate]
largestHelper candidate xs = if candidate == xs !! 0 then candidate : xs else if candidate > xs !! 0 then [candidate] else xs
