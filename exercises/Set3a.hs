...
firstHalf str = take (if isEven l then div l 2 else div (l+1) 2) str where l = length str
...
capitalizeFirst [] = []
capitalize str = unwords $ map (\w -> capitalizeFirst w) (words str)
...
whileRight f x = either (\x -> x) (\x -> whileRight f x) (f x)
...
catenated :: [String] -> [String]
catenated xs = [first ++ last | first <- xs, last <- xs]
joinToLength l list = filter (\x -> length x == l) (catenated list)
